...
from models.models import Endereco, db, Usuario, Perfil, Veiculo, TipoVeiculo, Cliente, VehiclePhoto, Token, Reserva
...
@main.route("/addadress/<int:user_id>",methods=["POST","GET"])
def adicionar_endereco(user_id):
    ...
    if not user:
        ...
        if request.method == "POST":
            ...
            return redirect(url_for("main.adicionar_endereco", user_id=user_id))
        return render_template("customer/add_adress.html",user=user)
